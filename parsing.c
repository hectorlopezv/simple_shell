...
/**
 *_which - return the path of a file if available
 *@collection_string: 2d array of tokeanized line
 *Return: return NULL if not found otherwise a string
 *
 */
...
char *_which(char **collection_string)
{
	...
	n_dir = 0, c_number = 0, path_mod = malloc(1), path_dup = malloc(1);
	check_path = NULL, size = _strlen(collection_string[0]);
	path = _getenv("PATH=");
	if (path != NULL)/* PATH= case empty*/
	{
		...
		if (check_path != NULL)
		{
			free(path_dup), free(path_mod), path_dup = NULL, path_mod = NULL;
	...
	free(path_dup), free(path_mod), path_dup = ft_strdup(path);/*malloc*/
	...
	set_path_mod(&path_mod, path_dup, largo_prueba), path_mod += 5;/*mody*/
	check_path = tokenizer_path_mod(&path_mod, size, collection_string);/*mlc*/
	if (path_dup != NULL)
