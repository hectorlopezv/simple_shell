...
/**
 *set_path_mod - fill path in the path_modifief
...
{
	char *path_pointer, *aux, *check_path;

	path_pointer = strtok(*path_mod, ":");

	while (path_pointer != NULL)
	{
		aux = malloc(_strlen(path_pointer) + size + 1);/*MALLOC*/
		if (!aux)
			return (aux);
		...
		path_pointer = strtok(NULL, ":");
		free(aux);
	}

	check_path = _check_cwd(command);  /* and is in cwd*/
	if (check_path != NULL)
		return (check_path);/*if cwd*/
